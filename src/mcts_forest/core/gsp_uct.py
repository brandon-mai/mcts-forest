...
@njit(cache=True)
def gsp_uct_core(n_sim, horizon, initial_state, c, gamma, p, rollout_limit, step_fn, rollout_fn, params, n_actions, T_s, Q_hat, V_hat, T_sa, state_h_to_node, node_states, node_counter, reward_offset, reward_scale):
    ...
    for _ in range(n_sim):
        ...
        while curr_h < horizon:
            ...
            if s_key in state_h_to_node:
                next_node = state_h_to_node[s_key]
            else:
                next_node = -1
            
            p_nodes[p_len], p_a[p_len], p_r[p_len] = curr_node, a, r_int
            p_len += 1
...
class GSPUCT:
    def __init__(self, env, c=1.0, p=2.0, horizon=100, gamma=0.99, rollout_limit=100, simulation_limit=1000, 
        ...
        try:
            ...
            raise RuntimeError(f"GSPUCT requires procedural dynamics: {e}")
    ...
    def search(self, initial_state: int) -> Tuple[int, Dict[str, Any]]:
        ...
        gsp_uct_core(self.simulation_limit, self.horizon, initial_state, self.c, self.gamma, self.p, self.rollout_limit, 
                    self.step_fn, self.rollout_fn, self.params, n_actions,
                    T_s, Q_hat, V_hat, T_sa, state_h_to_node, n_states, n_counter, self.reward_offset, self.reward_scale)
        
        return int(np.argmax(Q_hat[0])), {"root_v": float(V_hat[0])}

    def get_name(self) -> str:
        return f"gsp_uct_p{self.p}_sim{self.simulation_limit}"
