...
def jax_spuct_search(
    ...
    # Rollout function
    def rollout_fn(key, start_state, start_obs):
        ...
        (final_state, final_obs, final_key, reward_sum, _), _ = jax.lax.scan(
            rollout_step, (start_state, start_obs, key, 0.0, True), None, length=rollout_depth
        )
        return reward_sum
    ...
    # Define a single simulation step
    def simulation_step(carry, _):
        ...
        def forward_step(f_carry, _):
            ...
            def update_state_in_tree(states, idx, val):
                return jax.tree.map(lambda x, y: x.at[idx].set(y), states, val)
            
            new_t_states = jax.lax.cond(
                should_expand,
                lambda: update_state_in_tree(t_states, v, next_state),
                lambda: t_states
            )
        ...
        final_f_carry, trajectory = jax.lax.scan(forward_step, init_f_carry, None, length=max_depth)
        ...
        def backward_step(b_carry, step_data):
    ...
    final_carry, _ = jax.lax.scan(simulation_step, init_carry, None, length=num_simulations)
