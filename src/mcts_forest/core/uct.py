...
@njit(cache=True)
def uct_core(n_sim, horizon, c, gamma, rollout_limit, dynamics, visit_count, q_hat, action_visits, child_nodes, node_states, node_counter):
...
class UCT:
    ...
    def search(self, initial_state: int) -> Tuple[int, Dict[str, Any]]:
        ...
        uct_core(self.simulation_limit, self.horizon, self.c, self.gamma, self.rollout_limit, self.dynamics, v_count, q_hat, a_visits, child_nodes, n_states, n_counter)
